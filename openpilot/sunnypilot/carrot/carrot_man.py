...
class _NaviAppStatusControl(_NaviControlBase):
  def __init__(self, item: Any):
    present, sequence = _navi_meta(item)
    super().__init__(present, sequence)
    value = _navi_get(item, "value") if present else None
    if not isinstance(value, dict):
      value = {}
    self.foreground = _navi_bool(value, "foreground")
    self.window_focused = _navi_bool(value, "windowFocused")
    self.main_map_visible = _navi_bool(value, "mainMapVisible")
    self.ui_capture_available = _navi_bool(value, "uiCaptureAvailable")


class _NaviCameraStateControl(_NaviControlBase):
  def __init__(self, item: Any):
    present, sequence = _navi_meta(item)
    super().__init__(present, sequence)
    value = _navi_get(item, "value") if present else None
    if not isinstance(value, dict):
      value = {}
    self.camera_mode = _navi_text(value, "cameraMode")
    self.view_level = _navi_int(value, "viewLevel")
    self.view_sub_level = _navi_int(value, "viewSubLevel")
    self.tilt = _navi_float(value, "tilt", 0.0)
    self.bearing = _navi_float(value, "bearing", 0.0)


class _NaviCompositionStateControl(_NaviControlBase):
  def __init__(self, item: Any):
    present, sequence = _navi_meta(item)
    super().__init__(present, sequence)
    value = _navi_get(item, "value") if present else None
    if not isinstance(value, dict):
      value = {}
    self.generation = _navi_int(value, "generation")
    self.active = {k: v for k, v in value.items() if k != "generation" and isinstance(v, bool)}
...
class _CarrotNaviControl:
  def __init__(self, data: Any):
    ...
    self.app_status = _NaviAppStatusControl(_navi_get(data, "appStatus"))
    self.camera_state = _NaviCameraStateControl(_navi_get(data, "cameraState"))
    self.composition_state = _NaviCompositionStateControl(_navi_get(data, "compositionState"))
...
def _safe_int_or_none(value: Any, minimum: int | None = None, maximum: int | None = None) -> int | None:
  """Return int(value) if it is a finite number inside [minimum, maximum], else None."""
  try:
...
def _remaining_time(value: Any) -> int | None:
  return _safe_int_or_none(value, minimum=1, maximum=999)


def _traffic_light_debug_from_sinf(sinf: dict[str, Any]) -> dict[str, Any]:
  """Build a traffic-light debug dict from a sinf (single-direction) event."""
  return {
    "distanceM": _safe_int_or_none(sinf.get("distance"), minimum=0),
    "redS": _remaining_time(sinf.get("redLightRemainTime")),
    "straightS": _remaining_time(sinf.get("greenLightRemainTime")),
    "leftS": _remaining_time(sinf.get("leftLightRemainTime")),
    "rightS": _remaining_time(sinf.get("rightLightRemainTime")),
    "uturnS": _remaining_time(sinf.get("uturnLightRemainTime")),
...
def _traffic_light_debug_from_ssinf(ssinf: dict[str, Any]) -> dict[str, Any]:
  """Build a traffic-light debug dict from an ssinf (multi-direction) event."""
  red_remaining = []
  for signal_key, remain_key in (
    ...
    if str(ssinf.get(signal_key, "")).upper() == "RED_LIGHT_ON":
      remaining = _remaining_time(ssinf.get(remain_key))
      if remaining is not None:
        red_remaining.append(remaining)
  return {
    "distanceM": _safe_int_or_none(ssinf.get("distance"), minimum=0),
    "redS": max(red_remaining) if red_remaining else None,
    "straightS": _remaining_time(ssinf.get("straight_remain_time")),
    "leftS": _remaining_time(ssinf.get("left_remain_time")),
    "rightS": _remaining_time(ssinf.get("right_remain_time")),
    "uturnS": _remaining_time(ssinf.get("uturn_remain_time")),
...
class CarrotManager:
  ...
  def _apply_stock_nav_instruction(self, ni: Any) -> None:
    ...
    ni.maneuverType = _safe_str(getattr(stock, "maneuverType", ""), "")
    ni.maneuverModifier = _safe_str(getattr(stock, "maneuverModifier", ""), "")
    ni.distanceRemaining = float(getattr(stock, "distanceRemaining", 0.0) or 0.0)
    ...
    if stock_maneuvers:
      maneuvers = ni.init('allManeuvers', len(stock_maneuvers))
      for i, src in enumerate(stock_maneuvers):
  ...
  def _publish(self) -> None:
    ...
    cm.vehicleNaviActive = vehicle_navi_active
    ...
    # Fall back to stock navInstruction when Carrot phone nav is not active.
    if self._carrot_serv.active_carrot == 0 and self._stock_nav_instruction_active():
      self._apply_stock_nav_instruction(ni)
    else:
      secondary_text = _safe_str(raw.get("szNearDirName"), "")
      far_dir = _safe_str(raw.get("szFarDirName"), "")
      if far_dir:
        secondary_text += f"[{far_dir}]"
      ni.maneuverPrimaryText = _safe_str(raw.get("szTBTMainText"), "")
      ni.maneuverSecondaryText = secondary_text
      ...
      # Mirror cp logic: the second maneuver distance is the raw next-turn
      # distance (nTBTDistNext), not the accumulated x_dist_to_turn_next.
      # Emit it only when it is not closer than the current maneuver.
      n_tbt_dist = _safe_int(raw.get("nTBTDist"), 0)
      n_tbt_dist_next = _safe_int(raw.get("nTBTDistNext"), 0)
      has_next = x_turn_info_next > 0 and n_tbt_dist_next >= n_tbt_dist
      n_maneuvers = (1 if x_turn_info > 0 else 0) + (1 if has_next else 0)
      if n_maneuvers > 0:
  ...
  def get_radar_data(self) -> dict[str, Any]:
    ...
    Sources:
      * ``radarTracks`` (Car.RadarData) -> point cloud / tracks
    ...
    try:
      ...
      if self.sm.alive.get("radarTracks", False):
        ...
        for point in points:
          radar_data["points"].append({
            "id": int(getattr(point, "trackId", 0)),
            "dRel": float(getattr(point, "dRel", 0.0)),
            "yRel": float(getattr(point, "yRel", 0.0)),
            "vRel": float(getattr(point, "vRel", 0.0)),
          })
        # Car.RadarData does not have a separate tracks list; mirror points there.
      ...
      if self.sm.alive.get("radarState", False):
        ...
        for attr, label in (("leadOne", "leadOne"), ("leadTwo", "leadTwo")):
          ...
          if lead is None:
            continue
          present = bool(getattr(lead, "present", False))
          if not present:
            continue
          radar_data["leads"].append({
            "type": label,
            ...
            "radarTrackId": int(getattr(lead, "radarTrackId", -1)),
            "status": 1 if present else 0,
          })

      # 3. Four-corner + blind-spot state from AmapNaviServ.
      sd = self._amap_navi.shared_data
      radar_data["left_blind"] = sd.left_blind
      radar_data["right_blind"] = sd.right_blind
      radar_data["lidar_l"] = sd.lidar_left
      radar_data["lidar_r"] = sd.lidar_right
      radar_data["camera_l"] = sd.camera_left
      radar_data["camera_r"] = sd.camera_right
      radar_data["lf_drel"] = dict(sd.lf_drel)
      radar_data["lb_drel"] = dict(sd.lb_drel)
      radar_data["rf_drel"] = dict(sd.rf_drel)
      radar_data["rb_drel"] = dict(sd.rb_drel)
      radar_data["lf_vrel"] = sd.lf_vrel
      radar_data["lb_vrel"] = sd.lb_vrel
      radar_data["rf_vrel"] = sd.rf_vrel
      radar_data["rb_vrel"] = sd.rb_vrel
      radar_data["op_blocked"] = sd.op_blocked
      radar_data["road_blocked"] = sd.road_blocked
      radar_data["ext_blinker"] = sd.ext_blinker
    except Exception as e:
      cloudlog.error(f"carrot_man: get_radar_data error: {e}")
  ...
  def tick(self) -> None:
    ...
    if self.sm.alive.get('carParams', False):
  ...
  def _reset_carrot_navi_sequences(self, session_id: str) -> None:
    ...
    self._carrot_navi_app_status_sequence = -1
    self._carrot_navi_camera_state_sequence = -1
    self._carrot_navi_composition_state_sequence = -1
  ...
  def _apply_carrot_navi_sp(self) -> None:
    ...
    """
    if not self.sm.alive.get("carrotNaviSP", False):
      return
    try:
    ...
    # App/camera/composition status: compact text for webui / OP assistant.
    self._apply_carrot_navi_status(
      navi.app_status, "CarrotNaviAppStatus", "_carrot_navi_app_status_sequence",
      self._compact_app_status,
    )
    self._apply_carrot_navi_status(
      navi.camera_state, "CarrotNaviCameraState", "_carrot_navi_camera_state_sequence",
      self._compact_camera_status,
    )
    self._apply_carrot_navi_status(
      navi.composition_state, "CarrotNaviCompositionState", "_carrot_navi_composition_state_sequence",
      self._compact_composition_status,
    )
  ...
  @staticmethod
  def _compact_app_status(app: _NaviAppStatusControl) -> str:
    if not app.present:
      return "APP waiting"
    return "APP " + " ".join((
      "FG" if app.foreground else "BG",
      "FOCUS" if app.window_focused else "NO-FOCUS",
      "MAP" if app.main_map_visible else "NO-MAP",
      "CAPTURE" if app.ui_capture_available else "NO-CAPTURE",
    ))

  @staticmethod
  def _compact_camera_status(camera: _NaviCameraStateControl) -> str:
    if not camera.present:
      return "CAM waiting"
    mode = str(camera.camera_mode or "-").upper()
    return f"CAM {mode} L{camera.view_level}.{camera.view_sub_level} T{camera.tilt} B{camera.bearing}"

  @staticmethod
  def _compact_composition_status(composition: _NaviCompositionStateControl) -> str:
    if not composition.present:
      return "UI waiting"
    ...
    active = []
    for key, enabled in composition.active.items():
      if not enabled:
        continue
      label = key
      if label.lower().endswith("active"):
        label = label[:-6]
      active.append(_split_camel(label).strip().upper())
    return "UI " + (" / ".join(active[:5]) if active else "MAP ONLY")

  def _apply_carrot_navi_status(
    self,
    control: Any,
    param_key: str,
    sequence_attr: str,
    ...
    if not control.present:
      return
    if control.sequence == getattr(self, sequence_attr, -1):
      return
    setattr(self, sequence_attr, control.sequence)
    text = compact(control)
    try:
      self.params.put(param_key, text)
    except Exception as e:
      cloudlog.error(f"carrot_man: failed to write {param_key} param: {e}")
  ...
  def _handle_complex_crossroad(self, d: Any) -> None:
    ...
    # Severity mirrors the reference implementation: a visible complex crossroad
    # is treated as a caution, otherwise normal. An explicit `severity` from the
    # 7714 v2 crossroad payload takes precedence when present.
    crossroad_severity = d.get("severity")
    if not isinstance(crossroad_severity, str) or not crossroad_severity:
      crossroad_severity = "caution" if bool(d.get("show", False)) else "normal"

    # speedLimitKph / trafficLight are optional sub-structures carried by some
    # 7714 v2 crossroad payloads; default to None when absent (matching cp).
    raw_speed_limit = d.get("speedLimitKph")
    speed_limit_kph = _safe_int(raw_speed_limit, 0)
    if speed_limit_kph <= 0:
      speed_limit_kph = None
    raw_traffic_light = d.get("trafficLight")
    ...
    payload = {
      ...
      "severity": crossroad_severity,
      "speedLimitKph": speed_limit_kph,
      "trafficLight": traffic_light,
    }
    try:
      self.params_memory.put_nonblocking(NAVI_IMAGE_PARAM, payload)
    except Exception as e:
    ...
    try:
      ...
      if isinstance(obj, dict):
        if event_type == "rgdata" and isinstance(obj.get("rgdata"), dict):
          ...
          sinf = obj["sinf"]
          traffic_light = _traffic_light_debug_from_sinf(sinf)
          if sinf.get("redLightOn"):
            severity = "stop"
          ...
          ssinf = obj["ssinf"]
          traffic_light = _traffic_light_debug_from_ssinf(ssinf)
          ...
          crossroad_severity = crossroad.get("severity")
          if isinstance(crossroad_severity, str) and crossroad_severity:
            severity = crossroad_severity
          else:
            severity = "caution" if bool(crossroad.get("show", False)) else "normal"
  ...
  def _write_navi_debug(self) -> None:
    ...
    try:
      ...
      packet_fresh = last_packet > 0.0 and (now_mono - last_packet) < PACKET_TIMEOUT_SEC
      carrot_navi_active = getattr(self, "_carrot_navi_active", False)
      carrot_navi_has_control = getattr(self, "_carrot_navi_has_control", False)

      if carrot_navi_active and carrot_navi_has_control:
        active_source = "7714"
      ...
      if active_source == "stock":
        ...
        maneuver = {
          "type": _safe_str(getattr(stock, "maneuverType", "")),
          "modifier": _safe_str(getattr(stock, "maneuverModifier", "")),
          "distance": float(getattr(stock, "maneuverDistance", 0.0) or 0.0),
        }
      else:
        time_remaining = float(_safe_int(raw.get("nGoPosTime"), 0))
        distance_remaining = float(_safe_int(raw.get("nGoPosDist"), 0))
        maneuver = {
          "type": serv.nav_type,
          "modifier": serv.nav_modifier,
          "distance": serv.x_dist_to_turn,
          "xTurnInfo": serv.x_turn_info,
        }

      debug = {
        "receivedAt": time.strftime("%Y-%m-%dT%H:%M:%S"),
        "activeSource": active_source,
        "gpsSource": serv.gps_source,
        "epochTimeMs": serv.epoch_time,
        "timeRemaining": time_remaining,
        "distanceRemaining": distance_remaining,
        "maneuver": maneuver,
        "vehicleCanSpeedSources": {
          "camera": serv.vehicle_speed_camera_active(cs),
          "bump": serv.vehicle_speed_bump_active(cs),
          "school": serv.vehicle_school_zone_active(cs),
          "section": serv.vehicle_section_zone_active(cs),
        },
        "roadLimit": {
          "active": serv.n_road_limit_speed > 0,
          "speedKph": serv.n_road_limit_speed,
        },
        "offRoute": getattr(self, "_carrot_navi_off_route", False),
  ...
  def send_routes(self, coords: list, from_navd: bool = False) -> None:
    """Publish navRoute from the current navigation path.

    Mirrors CarrotPilot's send_routes(): converts incoming coordinate dicts
    (with keys like ``x``/``y`` or ``longitude``/``latitude``) into navRoute
    coordinates and, when from_navd is True, also stores them in
    ``self._navi_points`` for curvature-based speed calculations.
    """
    if not coords:
      return
    ...
    for c in coords:
      ...
      if isinstance(c, dict):
        if "longitude" in c and "latitude" in c:
          ...
          lon = c["x"]
          lat = c["y"]
      elif hasattr(c, "longitude") and hasattr(c, "latitude"):
        lon = c.longitude
        lat = c.latitude
      if lon is None or lat is None:
        continue
      ...
      if not (-180.0 <= lon_f <= 180.0 and -90.0 <= lat_f <= 90.0):
        continue
      route_coords.append({"latitude": lat_f, "longitude": lon_f})
      internal_points.append((lon_f, lat_f))

    if from_navd:
      if route_coords:
        self._navi_points = internal_points
        self._navi_points_start_index = 0
        self._navi_points_active = True
        self._navd_active = True
      else:
    ...
    msg.valid = True
    nav_route = msg.navRoute
    nav_route_coords = nav_route.init('coordinates', len(route_coords))
    for i, coord in enumerate(route_coords):
      nav_route_coords[i].latitude = coord["latitude"]
      nav_route_coords[i].longitude = coord["longitude"]
