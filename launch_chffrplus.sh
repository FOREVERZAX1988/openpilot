...
fix_opendbc_capnp_import() {
  local car_capnp="$DIR/opendbc_repo/opendbc/car/car.capnp"
  if [ -f "$car_capnp" ] && grep -q 'import "/include/c++.capnp"' "$car_capnp" 2>/dev/null; then
    sed -i 's#import "/include/c++.capnp"#import "./include/c++.capnp"#' "$car_capnp"
    echo "Patched opendbc car.capnp capnp import path"
  fi
}
