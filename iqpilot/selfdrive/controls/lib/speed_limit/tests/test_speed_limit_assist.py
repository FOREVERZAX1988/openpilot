...
from openpilot.iqpilot.selfdrive.controls.lib.speed_limit import PCM_LONG_REQUIRED_MAX_SET_SPEED
from openpilot.iqpilot.selfdrive.controls.lib.speed_limit.common import Mode
from openpilot.iqpilot.selfdrive.controls.lib.speed_limit.speed_limit_assist import SpeedLimitAssist, \
...
class TestSpeedLimitAssist:
  ...
  def setup_method(self, method):
    ...
    self.sla = SpeedLimitAssist(CI.CP, CI.CP_IQ)
  ...
  def _setup_platform(self, car_name):
    ...
    iqpilot_interfaces.setup_interfaces(CI, self.params)
    return CI

  def reset_custom_params(self):
    self.params.put("IsReleaseIqBranch", True)
  ...
  def test_disallowed_brands(self, car_name):
    ...
    for _ in range(int(PARAMS_UPDATE_PERIOD / DT_MDL)):
      self.sla.update(True, False, SPEED_LIMITS['city'], 0, SPEED_LIMITS['highway'], SPEED_LIMITS['city'],
                      SPEED_LIMITS['city'], True, 0, self.events_iq)
    assert not self.sla.enabled

  def test_disabled(self):
    self.params.put("SpeedLimitMode", int(Mode.off))
    for _ in range(int(10. / DT_MDL)):
      self.sla.update(True, False, SPEED_LIMITS['city'], 0, SPEED_LIMITS['highway'], SPEED_LIMITS['city'], SPEED_LIMITS['city'], True, 0, self.events_iq)
    assert self.sla.state == SpeedLimitAssistState.disabled
  ...
  def test_transition_disabled_to_pending_no_speed_limit_not_max_initial_set_speed(self):
    for _ in range(int(3. / DT_MDL)):
      self.sla.update(True, False, SPEED_LIMITS['highway'], 0, SPEED_LIMITS['city'], 0, 0, False, 0, self.events_iq)
  ...
  def test_preactive_to_active_with_max_speed_confirmation(self):
    ...
    self.sla.update(True, False, SPEED_LIMITS['city'], 0, self.pcm_long_max_set_speed, SPEED_LIMITS['highway'],
                    SPEED_LIMITS['highway'], True, 0, self.events_iq)
  ...
  def test_preactive_to_pending_no_speed_limit(self):
    self.sla.state = SpeedLimitAssistState.preActive
    self.sla.update(True, False, SPEED_LIMITS['highway'], 0, self.pcm_long_max_set_speed, 0, 0, False, 0, self.events_iq)
    assert self.sla.state == SpeedLimitAssistState.pending
    assert self.sla.is_enabled and not self.sla.is_active
  ...
  def test_pending_to_active_when_speed_limit_available(self):
    ...
    self.sla.update(True, False, SPEED_LIMITS['highway'], 0, self.pcm_long_max_set_speed,
                    SPEED_LIMITS['highway'], SPEED_LIMITS['highway'], True, 0, self.events_iq)
    assert self.sla.state == SpeedLimitAssistState.active
  ...
  def test_manual_cruise_change_detection(self):
    ...
    self.sla.update(True, False, SPEED_LIMITS['city'], 0, different_cruise, SPEED_LIMITS['city'], SPEED_LIMITS['city'], True, 0, self.events_iq)
    assert self.sla.state == SpeedLimitAssistState.inactive

  # TODO-IQ: test lower CST cases
  def test_rapid_speed_limit_changes(self):
    ...
    for _, speed_limit in enumerate(speed_limits):
      self.sla.update(True, False, speed_limit, 0, self.pcm_long_max_set_speed, speed_limit, speed_limit, True, 0, self.events_iq)
  ...
  def test_invalid_speed_limits_handling(self):
    ...
    for invalid_limit in invalid_limits:
      self.sla.update(True, False, SPEED_LIMITS['city'], 0, self.pcm_long_max_set_speed, invalid_limit, SPEED_LIMITS['city'], True, 0, self.events_iq)
  ...
  def test_stale_data_handling(self):
    ...
    self.sla.update(True, False, SPEED_LIMITS['city'], 0, self.pcm_long_max_set_speed, 0, old_speed_limit, True, 0, self.events_iq)
    assert self.sla.state in ACTIVE_STATES
  ...
  def test_distance_based_adapting(self):
    ...
    self.sla.update(True, False, current_speed, 0, self.pcm_long_max_set_speed, target_speed, target_speed, True, distance, self.events_iq)
    assert self.sla.state == SpeedLimitAssistState.adapting
    assert self.sla.output_v_target == target_speed  # TODO-IQ: assert expected accel, need to enable self.acceleration_solutions
  ...
  def test_maintain_states_with_no_changes(self):
    ...
    for state in test_states:
      ...
      self.sla.update(True, False, SPEED_LIMITS['city'], 0, self.pcm_long_max_set_speed, SPEED_LIMITS['city'], SPEED_LIMITS['city'], True, 0, self.events_iq)
