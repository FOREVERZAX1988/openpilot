...
from openpilot.iqpilot.selfdrive.controls.lib.speed_limit.helpers import compare_cluster_target, set_speed_limit_assist_availability
...
class SpeedLimitAssist:
  ...
  def __init__(self, CP: car.CarParams, CP_IQ: custom.IQCarParams):
    self.params = Params()
    self.CP = CP
    self.CP_IQ = CP_IQ
    self.frame = -1
    ...
    set_speed_limit_assist_availability(self.CP, self.CP_IQ, self.params)
    ...
    # TODO-IQ: SLA's own output_a_target for planner
  ...
  def update_active_event(self, events_iq: IQEvents) -> None:
    if self.v_cruise_cluster_below_confirm_speed_threshold:
      events_iq.add(EventNameIQ.speedLimitChanged)
    else:
      events_iq.add(EventNameIQ.speedLimitActive)
  ...
  def update_events(self, events_iq: IQEvents) -> None:
    if self.state == SpeedLimitAssistState.preActive:
      events_iq.add(EventNameIQ.speedLimitPreActive)

    if self.state == SpeedLimitAssistState.pending and self._state_prev != SpeedLimitAssistState.pending:
      events_iq.add(EventNameIQ.speedLimitPending)

    if self.is_active:
      if self._state_prev not in ACTIVE_STATES:
        self.update_active_event(events_iq)
  ...
  def update(self, long_enabled: bool, long_override: bool, v_ego: float, a_ego: float, v_cruise_cluster: float, speed_limit: float,
             speed_limit_final_last: float, has_speed_limit: bool, distance: float, events_iq: IQEvents) -> None:
