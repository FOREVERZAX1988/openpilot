...
GEARS_ALLOW_PAUSED_SILENT = [EventNameIQ.silentWrongGear, EventNameIQ.silentReverseGear, EventNameIQ.silentBrakeHold,
                             EventNameIQ.silentDoorOpen, EventNameIQ.silentSeatbeltNotLatched, EventNameIQ.silentParkBrake]
...
class AolStateMachine:
  ...
  def check_contains(self, event_type: str) -> bool:
    return bool(self._events.contains(event_type) or self._events_iq.contains(event_type))

  def check_contains_in_list(self) -> bool:
    return bool(self._events.contains_in_list(GEARS_ALLOW_PAUSED) or self._events_iq.contains_in_list(GEARS_ALLOW_PAUSED_SILENT))
  ...
  def update(self):
    ...
    if self.state != State.disabled:
      ...
      if self.check_contains(ET.USER_DISABLE):
        if self._events_iq.has(EventNameIQ.silentLkasDisable):
    ...
    # Check if AOL is engaged and actuators are enabled.
