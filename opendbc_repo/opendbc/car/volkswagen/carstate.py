...
class CarState(CarStateBase):
  ...
  CRUISE_FAULT_LATERAL_RECOVER_FRAMES = 300
  ...
  def update(self, can_parsers) -> tuple[structs.CarState, structs.CarStateSP]:
    ...
    if True:
      ...
      cc_only = self.CP.flags & VolkswagenFlagsSP.SP_CC_ONLY or self.CP.flags & VolkswagenFlagsSP.SP_CC_ONLY_NO_RADAR
    ...
    if hasattr(ret, "cruiseFaultLateralMode"):
  ...
  def update_pq(self, pt_cp, br_cp, cam_cp, ext_cp) -> tuple[structs.CarState, structs.CarStateSP]:
    ...
    ret.lateralAvailable = ret.cruiseState.available or self.cruise_fault_lateral_active
    ret.blockPcmEnable = self.cruise_fault_lateral_active and not ret.cruiseState.available
