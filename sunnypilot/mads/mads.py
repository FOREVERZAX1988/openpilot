...
class ModularAssistiveDrivingSystem:
  ...
  def _consume_joystick_mads_request(self, CS: structs.CarState) -> str | None:
    if not self.params.get_bool("JoystickDebugMode"):
      return None

    raw = self.params.get("JoystickMadsRequest")
    if not raw:
      return None
    ...
    self.params.remove("JoystickMadsRequest")
  ...
  def update_events(self, CS: structs.CarState):
    request = self._consume_joystick_mads_request(CS)
    if request == "enable":
