...
from openpilot.sunnypilot.common.slc_utilities import calculate_bearing_offset, is_url_pingable
from openpilot.sunnypilot.common.slc_variables import (
...
class SpeedLimitController:
  def __init__(self, params):
    """
    Initialize Speed Limit Controller.

    Args:
      params: Params instance for persistent storage
    """
    self.params = params
    ...
    # Load Mapbox request tracking
    mapbox_requests_raw = self.params.get("MapBoxRequests")
    if mapbox_requests_raw is not None:
      try:
  ...
  def get_offset(self, is_metric):
    """
    Get the appropriate speed offset for the current target speed.

    Args:
      is_metric: Boolean indicating if using metric units

    Returns:
      Speed offset in m/s for the current target speed bracket
    """
  ...
  def update_gps(self, sm):
    """
    Update GPS position and validity from SubMaster.

    Args:
      sm: SubMaster instance with GPS data
    """
    # Try gpsLocationExternal first, fallback to gpsLocation
    ...
    if gps_location and hasattr(gps_location, 'flags') and gps_location.flags > 0:
      self.gps_valid = True
      self.gps_position = {
  ...
  def get_mapbox_speed_limit(self, now, time_validated, v_ego, sm):
    """
    Fetch speed limit from Mapbox API asynchronously.
    ...
    if not self.gps_valid or not self.mapbox_token or (sm["carState"].steeringAngleDeg - sm["liveParameters"].angleOffsetDeg) >= 45:
      self.mapbox_limit = 0
      self.segment_distance = 0
      return
    ...
    def make_request():
      try:
        ...
        self.params.put_nonblocking("MapBoxRequests", json.dumps(self.mapbox_requests))
  ...
  def handle_limit_change(self, desired_source, desired_target, sm, slc_params):
    """
    Handle speed limit changes with confirmation/denial logic.

    Args:
      desired_source: Source of the new speed limit
      desired_target: New target speed limit in m/s
      sm: SubMaster instance
      slc_params: Dictionary of SLC parameters
    """
    self.speed_limit_changed_timer += DT_MDL

    # Check for acceptance via accel button or class variable
    accel_pressed = getattr(sm["carStateSP"], "accelPressed", False) if sm.alive.get("carStateSP", False) else False
    speed_limit_accepted = (accel_pressed and sm["carControl"].longActive) or self.speed_limit_accepted

    # Check for denial via decel button or 30s timeout
    decel_pressed = getattr(sm["carStateSP"], "decelPressed", False) if sm.alive.get("carStateSP", False) else False
    ...
    if speed_limit_accepted:
      ...
      self.speed_limit_accepted = False  # Reset after acceptance
  ...
  def update_limits(self, dashboard_speed_limit, now, time_validated, v_cruise, v_ego, sm, slc_params):
    """
    Update speed limits from all sources and apply priority logic.

    Args:
      dashboard_speed_limit: Speed limit from dashboard/CAN in m/s
      now: Current datetime
      time_validated: Boolean indicating if time is validated
    ...
    # Fetch Mapbox as primary source (with rate limiting)
    ...
    # Primary sources: Dashboard, Mapbox, Map Data (OSM as offline backup)
    limits = {
    ...
    if priority_highest:
      ...
      # Check priorities in order (Dashboard/Mapbox/Map Data depending on user config)
    ...
    # Fallback when no primary source available
  ...
  def update_map_speed_limit(self, v_ego, sm, slc_params):
    """
    Update map-based speed limit with lookahead logic.
    ...
    if sm.alive.get("liveMapDataSP", False):
      ...
      # Convert from m/s if valid, otherwise 0
  ...
  def update_override(self, v_cruise, v_cruise_diff, v_ego, v_ego_diff, sm, slc_params, is_metric):
    """
    Update manual override logic for speed limit control.

    Args:
      v_cruise: Current cruise speed in m/s
      v_cruise_diff: Difference between cluster and actual cruise speed
      v_ego: Current vehicle speed in m/s
      v_ego_diff: Difference between cluster and actual vehicle speed
      sm: SubMaster instance
      slc_params: Dictionary of SLC parameters
      is_metric: Boolean indicating if using metric units
    """
