...
_DEFAULT_API_HOST = os.getenv('API_HOST', 'https://api-konn3kt.teallvbs.xyz')
_DEFAULT_ATHENA_HOST = _DEFAULT_API_HOST.replace("https://", "wss://").replace("http://", "ws://")
ATHENA_HOST = os.getenv('ATHENA_HOST', _DEFAULT_ATHENA_HOST)
...
# TODO-SP: adapt zst for custom athena servers
...
def add_log_to_queue(log_path, log_id, is_custom_athena=False):
  MAX_SIZE_KB = 32
  ...
  with open(log_path) as f:
    ...
    if is_custom_athena and current_size > MAX_SIZE_BYTES:
    ...
    if is_custom_athena and is_compressed:
      jsonrpc["params"]["compressed"] = is_compressed

    jsonrpc_str = json.dumps(jsonrpc)
    size_in_bytes = len(jsonrpc_str.encode('utf-8'))

    if is_custom_athena and size_in_bytes <= MAX_SIZE_BYTES:
      cloudlog.debug(f"Target is custom athena and log file {log_path} is small enough to send in one request ({size_in_bytes} bytes).")
      low_priority_send_queue.put_nowait(jsonrpc_str)
    elif is_custom_athena:
      cloudlog.warning(f"Target is custom athena and log file {log_path} is too large to send in one request.")
    else:
      cloudlog.debug(f"Target is standard athena, proceeding to send log file {log_path} in one request ({size_in_bytes} bytes).")
      low_priority_send_queue.put_nowait(jsonrpc_str)


def log_handler(end_event: threading.Event, log_attr_name=LOG_ATTR_NAME) -> None:
  is_custom_athena = log_attr_name != LOG_ATTR_NAME
  if PC:
    ...
    try:
      ...
      if len(log_files) > 0:
        ...
        try:
          ...
          add_log_to_queue(log_path, log_entry, is_custom_athena)
          curr_log = log_entry
        except OSError:
...
def stat_handler(end_event: threading.Event, stats_dir=None, is_custom_athena=False) -> None:
...
def main(exit_event: threading.Event = None):
  if os.getenv("IQPILOT_ENABLE_ATHENAD", "0") != "1":
    cloudlog.warning("athenad disabled (set IQPILOT_ENABLE_ATHENAD=1 to re-enable for debugging)")
