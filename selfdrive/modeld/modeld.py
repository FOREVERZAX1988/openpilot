...
# [全域設定] 給 controlsd 參考的基礎延遲
LAT_SMOOTH_SECONDS = 0.1
LONG_SMOOTH_SECONDS = 0.3
MIN_LAT_CONTROL_SPEED = 0.3

# [CS 實際曲率平滑設定]
# 基於車輛實際 YawRate 計算出的曲率進行平滑
# 0.003 以下 (直線) 使用 0.1s 平滑 -> 極度穩定
# 0.008 以上 (大彎) 使用 0.02s 平滑 -> 反應迅速且過濾高頻噪訊
LAT_SMOOTH_BP = [0.003, 0.008]
...
def main(demo=False):
  ...
  while True:
    ...
    # [核心修改：使用 CS 實際偏航率計算曲率]
    # Curvature = YawRate / Velocity
    actual_curvature = abs(sm["carState"].yawRate) / max(v_ego, 0.5) 
    lat_smooth_seconds = float(np.interp(actual_curvature, LAT_SMOOTH_BP, LAT_SMOOTH_V))

    # 計算總延遲（僅用於計算預期位置，不影響 modeld 內部的平滑秒數）
