...
from openpilot.system.athena.registration import UNREGISTERED_DONGLE_ID, get_cached_dongle_id, ensure_dev_pairing_identity
...
class PrimeState:
  ...
  def __init__(self):
    ...
    # Must be computed at runtime (OPENPILOT_PREFIX can change Paths.persist_root()).
    self._konn3kt_state_path = Path(Paths.persist_root()) / "comma" / "konn3kt_prime_type"
  ...
  def _write_cached_state(self, prime_type: PrimeType) -> None:
    try:
      self._konn3kt_state_path.parent.mkdir(parents=True, exist_ok=True)
      self._konn3kt_state_path.write_text(str(int(prime_type)))
    except Exception:
  ...
  def _load_initial_state(self) -> PrimeType:
    ...
    try:
      ...
      pass
    try:
      if self._konn3kt_state_path.is_file():
        return PrimeType(int(self._konn3kt_state_path.read_text().strip()))
    except Exception:
