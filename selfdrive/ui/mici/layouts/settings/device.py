...
from openpilot.system.athena.registration import get_cached_dongle_id
from openpilot.system.athena.registration import ensure_dev_pairing_identity
...
class UpdateOpenpilotBigButton(BigButton):
  def __init__(self):
    ...
    super().__init__("update IQ.Pilot", "", self._txt_update_icon)
...
class ForceOffroadBigButton(BigButton):
  def __init__(self):
    super().__init__("force offroad", "", "icons_mici/settings/device/power.png", icon_size=(64, 66))

  def _update_state(self):
    forced_offroad = ui_state.params.get_bool("OffroadMode")
    if forced_offroad:
      self.set_text("force offroad on")
      self.set_value("tap to allow onroad")
    else:
      self.set_text("force offroad")
      self.set_value("tap to force offroad now")

  def _handle_mouse_release(self, mouse_pos: MousePos):
    super()._handle_mouse_release(mouse_pos)

    currently_forced = ui_state.params.get_bool("OffroadMode")
    force_offroad = not currently_forced
    ui_state.params.put_bool("OffroadMode", force_offroad)

    # Kick a start-state cycle when enabling so transition to offroad is immediate.
    if force_offroad and ui_state.started:
...
class DeviceLayoutMici(NavWidget):
  def __init__(self, back_callback: Callable):
    ...
    uninstall_openpilot_btn = BigButton("uninstall IQ.Pilot", "", "icons_mici/settings/device/uninstall.png")
    ...
    force_offroad_btn = ForceOffroadBigButton()
