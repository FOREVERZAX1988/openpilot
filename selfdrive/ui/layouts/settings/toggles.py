...
DESCRIPTIONS = {
  ...
  "SpeedLimitMode": tr_noop(
    "Controls IQ.Pilot speed limit behavior. Off disables speed limit features, Information only displays limits, Warning highlights overspeed, and Assist adjusts set speed using detected limits."
  ),
...
class TogglesLayout(Widget):
  def __init__(self):
    ...
    self._speed_limit_mode_setting = multiple_button_item(
      ...
      buttons=[lambda: tr("Off"), lambda: tr("Info"), lambda: tr("Warning"), lambda: tr("Assist")],
      button_width=220,
  ...
  def _update_toggles(self):
    ...
    if ui_state.CP is not None:
      ...
      self._toggles["LongitudinalControlMode"].set_visible(show_longitudinal_controls)
      self._long_personality_setting.set_visible(show_longitudinal_controls)

      if show_longitudinal_controls:
        mode_index = self._get_longitudinal_control_mode_index()
        self._toggles["LongitudinalControlMode"].action_item.set_selected_button(mode_index)
        self._toggles["LongitudinalControlMode"].action_item.set_enabled(selector_enabled and not ui_state.engaged)
        self._toggles["LongitudinalControlMode"].set_description(
          tr(DESCRIPTIONS["LongitudinalControlMode"]) + "<br><br>" + e2e_description
        )

        personality_enabled = selector_enabled and mode_index in (2, 3)
        self._long_personality_setting.action_item.set_enabled(personality_enabled)
    else:
      self._toggles["LongitudinalControlMode"].set_visible(False)
      self._long_personality_setting.set_visible(False)
  ...
  def _apply_longitudinal_control_mode(self, button_index: int):
    ...
    if previous_alpha != self._params.get_bool("AlphaLongitudinalEnabled"):
