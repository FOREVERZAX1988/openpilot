...
TSKKeyboard::TSKKeyboard() :
  ...
  QObject::connect(this, &ButtonControl::clicked, [=]() {
    ...
    QString archived2 = getArchive("/data/params/d/SecOCKey");
    if (!archived.length() && !archived2.length()) {
      // Old location
    ...
    } else if (archived2.length()) {
      // Old location
      defaultText = archived2;
    }

    // Show the archived key as a hint
    QString subtitle = "";
    if (archived.length()) {
      subtitle = QString(tr("Archived key: ")) +  archived;
    } else if (archived2.length()) {
      // Old location
      defaultText = archived2;
    }

    QString key = InputDialog::getText(tr("Enter your Car Security Key"), this, subtitle, false, 32, defaultText);
    if (key.length() != 0) {
      if (isValid(key)) {
...
void TSKKeyboard::refresh() {
  ...
  // 多元判断：检查是否没有任何密钥存在
  if (!installed.length() && !archived.length() && !archived2.length()) {
    setDescription(tr("Not Installed"));
  } else {
    // 至少有一个密钥存在，显示相应的信息
    if (installed.length()) {
      setDescription(tr("Archived key: ") + installed);
    } else if (archived.length()) {
      setDescription(tr("Archived key: ") + archived);
    } else {
      setDescription(tr("Archived key: ") + archived2);
    }
  }
  setEnabled(true);
}
