...
std::optional<bool> send_panda_states(PubMaster *pm, const std::vector<Panda *> &pandas, bool is_onroad, bool spoofing_started, bool always_offroad) {
  ...
  const bool allow_bench_power_save = getenv("IQPILOT_ALLOW_BENCH_POWERSAVE") != nullptr;
  ...
  bool bench_mode = !pandaStates.empty();
  for (const auto &health : pandaStates) {
    bench_mode &= (health.car_harness_status_pkt == (uint8_t)cereal::PandaState::HarnessStatus::NOT_CONNECTED);
  }
  ...
  for (uint32_t i = 0; i < pandas_cnt; i++) {
    ...
    // Keep panda awake on bench (no harness connected) for faster dev iterations.
    bool power_save_desired = (!allow_bench_power_save && bench_mode) ? false : !ignition_local;
